...
namespace Cj.AppEmbeddedApp.DAL
{
    public class TasksDAL
    {
        private const string strSql_insert_tasksign = @"insert into tasks (typeid,remotes,mobileindex,bodys,status,slidenums,repeatnums,friendnums,randommins,randommaxs,resultval,isstep,iswhole,
                                                        ...
                                                        @inputname,unix_timestamp(),@starttime)";

        private const string strSql_select_timedtasks = @"select id,typeid,remotes,mobileindex,bodys,status,resultval,slidenums,repeatnums,friendnums,randommins,randommaxs,inputname,isstep,iswhole,starttime,lastexecutetime from tasks where !ISNULL(starttime) && (ISNULL(lastexecutetime) || lastexecutetime < CURDATE()) order by starttime ";
        private const string strSql_select_globaltasks = @"select id,typeid,remotes,mobileindex,bodys,status,resultval,slidenums,repeatnums,friendnums,randommins,randommaxs,inputname,created,isstep,iswhole from tasks where `status`=@status and ISNULL(starttime) order by id limit @limit";

        private const string strSql_select_tasks_asc = @"select id,typeid,remotes,mobileindex,bodys,status,resultval,slidenums,repeatnums,friendnums,randommins,randommaxs,
                                                                    inputname,isstep,iswhole,starttime,lastexecutetime from tasks where (`status`=@status or @status='-1') order by id desc limit @limit";
        private const string strSql_select_tasks_desc = @"select id,typeid,remotes,mobileindex,bodys,status,resultval,slidenums,repeatnums,friendnums,randommins,randommaxs,
                                                                    inputname,isstep,iswhole,starttime,lastexecutetime from tasks where (`status`=@status or @status='-1') order by id desc limit @limit";
        private const string strSql_select_mobileindex_tasks_asc = @"select id,typeid,remotes,mobileindex,bodys,status,resultval,slidenums,repeatnums,friendnums,randommins,randommaxs,
                                                                    inputname,isstep,iswhole,starttime,lastexecutetime from tasks where (`status`=@status or @status='-1') and mobileindex=@mobileindex
        ...
        private const string strSql_update_updatetimedtask_executetime = @"update tasks set `lastexecutetime`=@lastexecutetime where id=@id";

        private const string strSql_delete_tasks = @"delete from tasks where id=@id or @id=-1";

        private const string strSql_delete_timedtasks = @"delete from tasks where starttime is null";
        private const string strSql_delete_globaltasks = @"delete from tasks where starttime != ''";
        ...
        {
            int flag = 0;
            MySqlParameter[] par = new MySqlParameter[15];
            ...
            par[14] = new MySqlParameter("@starttime", MySqlDbType.DateTime);
            par[14].Value = tasks.StartTime;
        ...
        public IList<TaskSch> GetGlobalTasks(string status, int limit = 100)
        {
            List<TaskSch> list = new List<TaskSch>();
            ...
            using (MySqlDataReader dr = MySqlHelpers.ExecuteReader(MySqlHelpers.ConnectionString, CommandType.Text, strSql_select_globaltasks,par))
            {
                while (dr.Read())
                {
                    TaskSch task = new TaskSch();
                    ...
                    task.IsWhole = Int32.Parse(dr["iswhole"].ToString());
                    task.InputName = dr["inputname"].ToString();
                    task.Created = Int32.Parse(dr["created"].ToString());
