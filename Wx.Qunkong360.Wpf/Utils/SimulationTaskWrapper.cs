...
namespace Wx.Qunkong360.Wpf.Utils
{
    public static class SimulationTaskWrapper
    {
        ...
        public static async Task<bool> GotoHomepage(int mobileIndex)
        {
            ...
            string search = SystemLanguageManager.Instance.ResourceManager.GetString("Search", SystemLanguageManager.Instance.CultureInfo);
            string message = SystemLanguageManager.Instance.ResourceManager.GetString("HomepageMessaging", SystemLanguageManager.Instance.CultureInfo);
            ...
            while (true)
            {
                ...
                if (await FuzzySearch(mobileIndex, search) && await FuzzySearch(mobileIndex, message))
                {
                    result = true;
                    break;
                }
