...
namespace Xzy.EmbeddedApp.Utils
{
    public static class ProcessUtils
    {
        ...
        public static void Init(int index, Simulator simulator)
        {
            ...
            string cmdImei = $"--imei {simulator.Imei}";
            string androidid = $"--androidid {simulator.Androidid}";
            //string PhoneBrand = $"--PhoneBrand {simulator.PhoneBrand}";
            //string PhoneType = $"--PhoneType {simulator.PhoneType}";
            string cmdStr = $"{LDPath}/dnconsole.exe modify --index {index} {cmdResolution} {cmdCpu} {cmdMemory} {androidid}";
            //string cmdStr = $"{LDPath}/dnconsole.exe modify --index {index} {cmdResolution} {cmdCpu} {cmdMemory} {androidid}{cmdImei}{PhoneBrand} {PhoneType}";
            CmdUtils.RunCmd(cmdStr);
        }
        ...
        public static int LaunchAndroidCommander(string device)
        {
            ...
            //string cmd = $"{LDPath}/adb.exe -s {device} shell am instrument -w -r   -e debug false -e class test.InstrumentationTest#testFunction com.kcm.uiauto/android.support.test.runner.AndroidJUnitRunner";
